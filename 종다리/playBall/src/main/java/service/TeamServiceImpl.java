...
public class TeamServiceImpl implements TeamService {
	...
	public boolean registerTeam(TeamVo vo) {//팀등록
		boolean bool = false;
		int r = teamMapper.registerTeam(vo);
		if(r>0) {
			bool = true;
		} else {
			bool = false;
	...
	public int deleteMember(TeamMemberVo vo) {
		int r = 0;
	...
	public int dismantleTeam(String tid) {
		int r = 0;
	...
	public boolean checkJoin(String mid, int serial){//가입전 중복확인
		...
		jVo.setTid(tid);
		jVo.setMid(mid);
		int r = teamMapper.checkJoinList(jVo);
		if(r> 0) {
			bool = false;
		}else {
			bool =true;
	...
	public int updateLeader(TeamVo vo){//주장 업데이트
		int r = teamMapper.updateLeader(vo);
		return r;
	}
