...
<div id = 'team'>
	<form name = 'frm_team' id = 'frm_team'>
		...
		<br/>
		<textarea name = 'intro' readOnly>${tvo.intro }</textarea>
		<br/>
		...
		<div id = "matching">
			...
			<div id='pageMatch'>
				<c:if test="${page.startPage>1 }">
					<input type='button' value='&lt&lt' onclick='team.recordMove(1)'/>
					<input type='button' value='&lt' onclick='team.recordMove(${page.startPage-1 })'/>
				</c:if>
				
				<c:forEach var='p' begin='${page.startPage }' end='${page.endPage }'>
					<input type='button' value='${p }' onclick ='team.recordMove(${p})' />
				</c:forEach>
				
				<c:if test="${page.endPage<page.totPage }">
					<input type='button' value='&gt' onclick ='team.recordMove(${page.endPage+1})'/>
					<input type='button' value='&gt&gt' onclick ='team.recordMove(${page.totPage})'/>
				</c:if>			
			</div>
		</div>
		...
		<div id = 'btn_zone'>
			...
			<c:choose>
				<c:when test="${sessionScope.tid eq tvo.tid}">
					...
					<input type = 'button' id = 'btnJoinList' onclick = "funcJoinList('${param.serial}')" value = '신청목록'/>
				</c:when>
